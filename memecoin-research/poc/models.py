"""Database schema for the proof of concept.

A deliberate subset of the Phase 1 design -- enough to prove the pipeline
stores data correctly, not the full collector. The parts that are here are
built the way the real thing will be, because those are what the PoC must
prove: chain-agnostic keys, merged price+liquidity observations, idempotent
writes, and raw payloads kept for reproducibility.
